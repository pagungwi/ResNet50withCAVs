...
class block(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.identity_downsample is not None:
            identity = seld.identity_downsample(identity)
...
Class ResNet(nn.Module): # [3, 4, 6, 3] for ResNet 50
        def __init__(self, block, layers, image_channels, numb_classes):
        ...
        def _make_layers(self, block, num_residual_blocks, out_channels, stride):
            ...
            if stride != 1 or self.in_channels != out_channels = 4:
                identity_downsample = nn.Sequential(nn.Conv2d(self.in_channels, out_channels=4, kernel_size=1, stride=stride), nnBatchNorm2d(out_channels*4))
            
            layaers.append(block(self.in_channels, out_channels, identity_downsample, stride))
            self.in_channels = out_channels*4 
            
            for i in range(num_residual_blocks - 1):
                layers.append(block(self.in_channels, out_channels)) #256 -> 64*4(256)
